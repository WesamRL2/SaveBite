...
public class AddSurplusProductPanel extends JPanel {
    ...
    private JPanel createFormPanel() {
        ...
        sellerIdField = createField("S001");
        nameField = createField("");
        originalPriceField = createField("");
        quantityField = createField("");
        categoryField = createField("");
        discountField = createField("");
        pickupHoursField = createField("");

        int row = 0;

        addFormRow(panel, gbc, row++, "Seller ID", sellerIdField);
        addFormRow(panel, gbc, row++, "Product Name", nameField);
        addFormRow(panel, gbc, row++, "Original Price (RM)", originalPriceField);
        addFormRow(panel, gbc, row++, "Quantity", quantityField);
        addFormRow(panel, gbc, row++, "Category", categoryField);
        addFormRow(panel, gbc, row++, "Discount (%)", discountField);
        addFormRow(panel, gbc, row++, "Pickup Within (Hours)", pickupHoursField);
    ...
    private JTextField createField(String text) {
        ...
        JTextField field = new JTextField(text, 22);
    ...
    private void addProduct() {
        ...
        try {
            ...
            String sellerId = ValidationUtil.requireText(
                    sellerIdField.getText(),
                    "Seller ID"
            );

            String name = ValidationUtil.requireText(
                    nameField.getText(),
                    "Product name"
            );

            double originalPrice = ValidationUtil.parsePositiveDouble(
            ...
            String category = ValidationUtil.requireText(
                    categoryField.getText(),
                    "Category"
            );

            double discount = ValidationUtil.parseDiscount(
                    discountField.getText()
            );

            int pickupHours = ValidationUtil.parsePositiveInt(
                    pickupHoursField.getText(),
                    "Pickup hours"
            );

            String productId = marketplaceService.generateProductId();
            ...
            Product product = new FoodProduct(
                    ...
                    category,
                    discount,
                    LocalDateTime.now().plusHours(pickupHours)
            );

            boolean added = marketplaceService.addProduct(product);
            ...
            try {
                ...
                ActivityLogger.logProductAdded(product);

            } catch (IOException e) {

                marketplaceService.removeProductById(productId);
